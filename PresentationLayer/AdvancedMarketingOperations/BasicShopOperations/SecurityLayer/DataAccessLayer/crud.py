...
def get_product(db: Session, product_id: int):
    return db.query(mProduct).filer(mProduct.id==product_id).first()
...
def create_order(db: Session, order: Order):
    db_order= models.Order(user_id=order.owner_id, items=order.products, isFinished=order.isFinished)
    db.add(db_order)
    db.commit()
    db.refresh(db_order)
    return db_order
